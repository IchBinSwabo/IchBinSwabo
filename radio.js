...
client.on("ready", () => {
    ...
    client.user.setActivity("🎶 Miljacka Radio 🎶", { type: "LISTENING" });
...
client.on(`message`, async (message) => {
    ...
    if(message.content.startsWith(`..naxiradio`)) // Naxi radio
    {
        ...
        client.user.setActivity("🎶 Naxi Radio 🎶", { type: "LISTENING" });
    ...
    if(message.content.startsWith(`,coolradio`)) // Cool radio
    {
        ...
        client.user.setActivity("🎶 Cool Radio 🎶", { type: "LISTENING" });
        dispatcher.on("end", end => { reloadradiobot() });
    }
    if(message.content.startsWith(`..bnradio`)) // BN radio
    {
        ...
        client.user.setActivity("🎶 BN Radio 🎶", { type: "LISTENING" });
        dispatcher.on("end", end => { reloadradiobot() });
    }
    if(message.content.startsWith(`,m`)) // Miljacka Radio
    {
        try{
            await voiceChannel.leave();
            await delay(300);
        ...
        var connection = await voiceChannel.join();
        await connection.voice.setSelfDeaf(true); await connection.voice.setDeaf(true);
        const dispatcher = connection.play('http://radiomiljacka-bhcloud.radioca.st/stream.mp3'); // pa bebo ti radis o.o
        client.user.setActivity("🎶 Miljacka Radio 🎶", { type: "LISTENING" });
        dispatcher.on("end", end => { reloadradiobot() });
    }
